...
if (isset($_GET['likedislike']) AND isset($_GET['id']) AND !empty($_GET['likedislike']) AND !empty($_GET['id'])) {
    $getId = (int) $_GET['id'];
    ...
    if ($checkPartner->rowCount() > 0) {
        ...
        if ($getLikeDislike == 1) {
            ...
            $checkLike->execute([$getId,$_SESSION['id']]);
            ...
            $delete->execute([$getId,$_SESSION['id']]);
            ...
            if ($checkLike->rowcount() == 1) {
                ...
                $insert->execute([$getId,$_SESSION['id']]);
            }
...
// COUNT COMMENTS
$req = $bdd->query('SELECT COUNT(*) AS total FROM comment LEFT JOIN partner ON comment.id_partner = partner.id WHERE id_partner = '.$_GET['partner'].''); 
...
$req->execute([$_SESSION['id'],$_GET['partner']]);
